(function(){
        ...
        const STORAGE_KEY = "nexus_dashboard_shortcuts_v4";
        const COLOR_KEY = "nexus_font_color";
        const BG_KEY = "nexus_background_style_v2";
        ...
        function setBackgroundImage(imageUrl) {
            if (!imageUrl) {
                ...
                if (bgPreviewStatus) bgPreviewStatus.innerHTML = "✅ Fondo restablecido a oscuro";
                showToast("Fondo restablecido", 1500);
                return;
            }
            ...
            img.onload = function() {
                ...
                applyBackground(imageUrl);
                showToast("✅ Fondo actualizado correctamente", 1500);
                if (bgPreviewStatus) bgPreviewStatus.innerHTML = "✅ Fondo cargado exitosamente";
            };
        ...
        // ---------- OBTENER ICONO REAL DEL SITIO (FAVICON) ----------
        ...
        function renderShortcuts() {
            ...
            shortcuts.forEach((item, idx) => {
                ...
                card.setAttribute("aria-label", `Abrir ${escapeHtml(item.title)}`);
                
                card.innerHTML = `
                    <img class="favicon-img" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23999'%3E%3Ccircle cx='12' cy='12' r='10'/%3E%3C/svg%3E" alt="Favicon de ${escapeHtml(item.title)}" data-site-url="${escapeHtml(item.url)}">
                    <div class="shortcut-title">${escapeHtml(item.title)}</div>
                    <div class="shortcut-url">${escapeHtml(truncateUrl(item.url))}</div>
                    <div class="card-actions">
                        <i class="fas fa-edit" data-action="edit" data-id="${item.id}" role="button" tabindex="0" aria-label="Editar ${escapeHtml(item.title)}"></i>
                        <i class="fas fa-trash-alt" data-action="delete" data-id="${item.id}" role="button" tabindex="0" aria-label="Eliminar ${escapeHtml(item.title)}"></i>
                    </div>
                `;
                ...
                card.addEventListener("keydown", (e) => {
                    if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        if(!e.target.closest(".card-actions")) window.open(item.url, "_blank");
                    }
                });
        ...
        bgFileInput.addEventListener("change", (e) => {
            ...
            if (file && file.type.startsWith("image/")) {
                ...
                reader.onload = (ev) => { 
                    ...
                    if (bgUrlInput) bgUrlInput.value = "📁 Imagen local cargada";
                    showToast("Fondo actualizado (imagen local)", 1500);
                };
                reader.readAsDataURL(file);
            } else { 
                showToast("Selecciona una imagen válida", 1500);
            }
        });
        ...
        setBgUrlBtn.addEventListener("click", () => {
            ...
            if (url) { 
                ...
                showToast("Ingresa URL de imagen (JPG, PNG, WEBP, GIF)", 2000);
            }
        });
        ...
        applyFontColorBtn.addEventListener("click", () => { 
            applyFontColorToAll(fontColorPicker.value); 
            showToast("Color de texto actualizado", 1000);
        });
